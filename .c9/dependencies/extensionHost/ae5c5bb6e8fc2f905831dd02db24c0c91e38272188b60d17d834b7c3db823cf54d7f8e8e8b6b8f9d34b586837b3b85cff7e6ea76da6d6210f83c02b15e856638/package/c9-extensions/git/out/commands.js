...
class CommandCenter {
    ...
    async setLogLevel() {
        ...
        this.outputChannel.appendLine(localize('changed', "{0} Log level changed to: {1}", (0, util_1.logTimestamp)(), log_1.LogLevel[log_1.Log.logLevel]));
    }
    ...
    async stage(...resourceStates) {
        this.outputChannel.appendLine(`${(0, util_1.logTimestamp)()} git.stage ${resourceStates.length}`);
        resourceStates = resourceStates.filter(s => !!s);
        if (resourceStates.length === 0 || (resourceStates[0] && !(resourceStates[0].resourceUri instanceof vscode_1.Uri))) {
            const resource = this.getSCMResource();
            this.outputChannel.appendLine(`${(0, util_1.logTimestamp)()} git.stage.getSCMResource ${resource ? resource.resourceUri.toString() : null}`);
            if (!resource) {
                return;
            }
        ...
        this.outputChannel.appendLine(`${(0, util_1.logTimestamp)()} git.stage.scmResources ${scmResources.length}`);
    ...
    getSCMResource(uri) {
        ...
        this.outputChannel.appendLine(`${(0, util_1.logTimestamp)()} git.getSCMResource.uri ${uri && uri.toString()}`);
        for (const r of this.model.repositories.map(r => r.root)) {
            this.outputChannel.appendLine(`${(0, util_1.logTimestamp)()} repo root ${r}`);
        }
        if (!uri) {
            return undefined;
        }
