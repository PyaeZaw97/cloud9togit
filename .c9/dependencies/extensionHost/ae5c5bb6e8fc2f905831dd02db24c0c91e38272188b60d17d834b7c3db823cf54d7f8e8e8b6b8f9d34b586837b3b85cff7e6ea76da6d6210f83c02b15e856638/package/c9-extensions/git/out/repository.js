...
class FileEventLogger {
    ...
    onDidChangeLogLevel(level) {
        ...
        this.eventDisposable = (0, util_1.combinedDisposable)([
            this.onWorkspaceWorkingTreeFileChange(uri => this.outputChannel.appendLine(`${(0, util_1.logTimestamp)()} [debug] [wt] Change: ${uri.fsPath}`)),
            this.onDotGitFileChange(uri => this.outputChannel.appendLine(`${(0, util_1.logTimestamp)()} [debug] [.git] Change: ${uri.fsPath}`))
        ]);
    }
    dispose() {
        this.eventDisposable.dispose();
...
class DotGitWatcher {
    constructor(repository, outputChannel) {
        ...
        const filteredRootWatcher = (0, util_1.filterEvent)(rootWatcher.event, uri => !/\/\.git(\/index\.lock)?$|\/\.watchman-cookie-/.test(uri.path));
    ...
    updateTransientWatchers() {
        ...
        try {
            ...
            if (log_1.Log.logLevel <= log_1.LogLevel.Error) {
                this.outputChannel.appendLine(`${(0, util_1.logTimestamp)()} Warning: Failed to watch ref '${upstreamPath}', is most likely packed.`);
            }
        }
    }
    dispose() {
    ...
    resolveChangeCommand(resource) {
        ...
        if (!resource.leftUri) {
            return {
                command: 'vscode.open',
                title: localize('open', "Open"),
                arguments: [resource.rightUri, { override: resource.type === 16 /* Status.BOTH_MODIFIED */ ? false : undefined }, title]
            };
        }
        else {
            return {
...
class Repository {
    constructor(repository, pushErrorHandlerRegistry, remoteSourcePublisherRegistry, globalState, outputChannel, telemetryReporter) {
        this.repository = repository;
        ...
        try {
            ...
            if (log_1.Log.logLevel <= log_1.LogLevel.Error) {
                outputChannel.appendLine(`${(0, util_1.logTimestamp)()} Failed to watch path:'${this.dotGit.path}' or commonPath:'${this.dotGit.commonPath}', reverting to legacy API file watched. Some events might be lost.\n${err.stack || err}`);
            }
